package com.techmatrix18.user_photo.application.port.in;

import com.techmatrix18.user_photo.application.command.UploadUserPhotoCommand;
import com.techmatrix18.user_photo.domain.model.UserPhoto;

/**
 * Inbound port defining the scenario for uploading a new profile photo.
 *
 * @author Alexander Kuziv <makklays@gmail.com>
 * @company TechMatrix18
 * @version 0.0.1
 * @since 17.08.2026
 */

public interface UploadUserPhotoUseCase {

    // Processes photo registration inside the domain core.
    UserPhoto execute(UploadUserPhotoCommand command);
}

